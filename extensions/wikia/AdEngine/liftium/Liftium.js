	...
	try {
		...
		for (var i in qcseg.segments) {
			...
			if (Liftium.e(qcseg.segments[i].id)) {
				...
				//WikiaTrackerQueue.track(Liftium.buildTrackUrl([LiftiumOptions.pubid, "quantcast", "segments", "broken", c]), 'liftium.errors');
				continue;
			}
			Liftium.d("Quantcast segment: " + qcseg.segments[i].id, 5);
			//FIXME: this tracking call needs to be updated before it is uncommented
			//WikiaTrackerQueue.track(Liftium.buildTrackUrl([LiftiumOptions.pubid, "quantcast", "segments", qcseg.segments[i].id]), 'liftium.quantcast');
		...
		//FIXME: this tracking call needs to be updated before it is uncommented
		//WikiaTrackerQueue.track(Liftium.buildTrackUrl([LiftiumOptions.pubid, "quantcast", "broken"]), 'liftium.errors');
		return;
	}
};
